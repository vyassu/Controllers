...
class VirtualNetworkInitializer(ControllerModule):
    ...
    def initialize(self):
        ...
        # Set Tincan LogLevel
        self.registerCBT("Logger", "info", "Setting Tincan log level to " + self.CONFIG["LogLevel"])
        lgl = ipoplib.LOGLVEL
        lgl["IPOP"]["Request"]["LogLevel"] = self.CONFIG["LogLevel"]   # LogLevel obtained from Config File
        ...
        for i in range(len(self.vnetdetails)):
            ...
            # Send GET Local State Request
            self.registerCBT("TincanInterface", "DO_SEND_TINCAN_MSG",get_state_request)
